...
func TestMinimumReadableTableWidthsUseDatasetAndPreferredWidths(t *testing.T) {
	cols := agentActivityTableColumns()
	rows := agentActivityTableRows()

	got := minimumReadableTableWidths(cols, rows)
	want := []int{12, 11, 12, 28, 6}
	if len(got) != len(want) {
...
func TestPlanTableLayoutSwitchesToMasterDetailBeforeReadableFloorViolation(t *testing.T) {
	...
	if plan.Mode != TableModeMasterDetail {
		t.Fatalf("expected master/detail once tabular compression would violate a readable floor, got %v", plan.Mode)
	}
}
