...
1. Focus `p03-services`, move to any row, then press `Enter` once. The frame
   immediately contains `✓ Selected: <service>` followed by the live spinner
   `⠋ Waiting for agent…`. The normal table footer is replaced while waiting.
   Move the table cursor again: the `Selected: <service>` line must retain the
   committed service rather than following the cursor. Exactly one `select`
   event is available; another event read times out.
2. Press `Tab` to focus `p03-note`, type a note, then press `Enter` once. The
   frame immediately contains `✓ Submitted: <value>` and the live `Waiting
   for agent…` spinner; normal input help is hidden. Exactly one `submit` event
   is available.
3. With a non-input focus, press `r` once. The frame immediately contains
   `Retry requested` and `Waiting for agent…`; exactly the existing one action
...
4. Publish and commit any ordinary V1 update. The local acknowledgement and
   waiting text disappear on that next accepted publication, and ordinary
   focus-specific footer help returns.
